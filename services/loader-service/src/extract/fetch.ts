...
export async function fetchGames(cup: boolean): Promise<Game[] | null> {
    try {
        ...
        const endpoint =
            config.SWISSVOLLEY_API_GAMES_ENDPOINT + (cup ? '?includeCup=1' : '')
        ...
        const vbcGames = await fetchFromSwissvolleyAPI<Game[]>(
            endpoint,
            vbcApiKey
        )
        const volleyGames = await fetchFromSwissvolleyAPI<Game[]>(
            endpoint,
            volleyApiKey
        )
...
export async function fetchRankings(): Promise<Ranking[] | null> {
    try {
        const rankings = await fetchFromSwissvolleyAPI<Ranking[]>(
            config.SWISSVOLLEY_API_RANKINGS_ENDPOINT,
            process.env.SWISSVOLLEY_API_KEY_VBCUNIBERN!
        )
        console.log(
...
export async function fetchGamesSeparated(
    cup: boolean
): Promise<[Game[], Game[]] | [null, null]> {
    try {
